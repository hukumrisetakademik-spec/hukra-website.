...
export async function generateMetadata({ params }: Props) {
  try {
    ...
    return { title: `${(data as any).title} — HUKRA`, description: (data as any).excerpt }
...
export default async function ArticlePage({ params }: Props) {
  try {
    ...
    // Get related articles - simple query, no join issues
    const { data: related } = await supabase
      .from('articles')
      .select('id, title, slug, type, cover_image')
      .eq('status', 'published')
      .neq('id', a.id)
      .limit(3)
                  ...
                  {(related as any[]).map(r => (
                    <Link key={r.id} href={`/artikel/${r.slug}`} className="flex gap-3 group">
                      {r.cover_image ? (
                        ...
                        <div className="w-16 h-14 rounded-lg shrink-0 flex items-center justify-center text-2xl" style={{ background: '#EFF4FF' }}>⚖️</div>
                      )}
                      <div>
